...
var cryptoNight;
if (!multiHashing || !multiHashing[cnAlgorithm]) {
    log('error', logSystem, 'Invalid cryptonight algorithm: %s', [cnAlgorithm]);
} else {
    cryptoNight = multiHashing[cnAlgorithm];
}
...
BlockTemplate.prototype = {
    nextBlob: function(){
        ...
        return cnUtil.convert_blob(this.buffer, cnBlobType).toString('hex');
    }
};
...
function processShare(miner, job, blockTemplate, nonce, resultHash){
    ...
    var shareBuffer = cnUtil.construct_block_blob(template, new Buffer(nonce, 'hex'), cnBlobType);
    ...
    if (shareTrustEnabled && miner.trust.threshold <= 0 && miner.trust.penalty <= 0 && Math.random() > miner.trust.probability){
        ...
        convertedBlob = cnUtil.convert_blob(shareBuffer, cnBlobType);
    ...
    if (hashDiff.ge(blockTemplate.difficulty)){
        ...
        apiInterfaces.rpcDaemon('submitblock', [shareBuffer.toString('hex')], function(error, result){
            if (error){
                ...
                var blockFastHash = cnUtil.get_block_id(shareBuffer, cnBlobType).toString('hex');
                log('info', logSystem,
