    ...
    async.waterfall([
        ...
        function(blocks, callback){
            ...
            blocks.forEach(function(block){
                ...
                /*
                 if (Object.keys(donationsNiobio).length) {
                    for(var wallet in donationsNiobio) {
                        var percent = donationsNiobio[wallet] / 100;
                        feePercent += percent;
                        payments[wallet] = Math.round(block.reward * percent);
                        log('info', logSystem, 'Block %d donation Nióbio to %s as %d percent of reward: %d', [block.height, wallet, percent, payments[wallet]]);
                    }
                }
                */
                // Take out fee for development and research wallets
                feePercent += 0.1;
